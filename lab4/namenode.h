...
class NameNode
{
  struct LocatedBlock
  {
    ...
    LocatedBlock(blockid_t block_id,
                 uint64_t offset,
                 uint64_t size,
                 const DatanodeIDProto &loc) : block_id(block_id),
                                               offset(offset),
                                               size(size),
                                               locs(1, loc) {}
    LocatedBlock(blockid_t block_id,
                 uint64_t offset,
                 uint64_t size,
                 const std::list<DatanodeIDProto> &locs) : block_id(block_id),
                                                           offset(offset),
                                                           size(size),
                                                           locs(locs) {}
  };
...
private:
  ...
  std::list<DatanodeIDProto> datanode_ids;
  std::list<blockid_t> written_blocks;
  enum State
  {
    live = 1,
    dead,
  ...
  struct DatanodeState
  {
    ...
    DatanodeState(State s,
                  unsigned int t) : state(s),
                                    lastHeartbeatTime(t) {}
  };

  std::map<DatanodeIDProto, DatanodeState> datanode_states;
  void CheckDataNodeState(DatanodeIDProto id);
  void ReplicateData(DatanodeIDProto id);
  void RecordWritten(yfs_client::inum ino);
